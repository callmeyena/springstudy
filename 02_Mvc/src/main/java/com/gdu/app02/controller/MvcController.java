...
public class MvcController {
	...
	*/
	...
	/*
	 	...
	 	메소드 마다 request.setCharacterEncoding("UTF-8")을 작성하는 것은 매우 비효율적이므로,
	 	모든 요청(contextPath를 가진 모든 요청)마다 동작하도록 filter를 사용한다.
	 	CharacterEncodingFilter를 통해서 모든 요청마다 자동으로 UFT-8로 인코딩 된다.
	...
	public String detailGdu(Person p) {		// name과 age를 필드로 가진 커맨드 객체 Person p
		return "mvc/detail";				// Model에 저장될 때 객체 이름인 p를 사용하지 않고, 객체 타입인 Person을 사용한다.
											// Model에 저장되는 속성명은 객체 타입 Person을 person으로 수정해서 사용한다.
	}
	
	/*
	 @GetMapping("/detail.gdu")
	 public String detailGdu(@ModelAttribute(value="p") Person person) {	// Mdoel에 저장할 속성명을 p로 해주셈!
