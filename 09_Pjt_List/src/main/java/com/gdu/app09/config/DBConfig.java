...
@PropertySource(value={"classpath:application.properties"})  // 프로퍼티 파일을 읽어온다 => application.properties 파일의 속성을 읽어 오자!
...
public class DBConfig {
	...
	// HikaryConfig Bean (application properties 값을 불러와서 여기로 두겠다/ DB property를 분리한 이유는 gitignore파일을 쓰지 않기 위해서(application.properties는 깃에 안올릴거임)
	@Bean
	...
	// HikariDataSource Bean (DriverManagerDataSource를 대체)
