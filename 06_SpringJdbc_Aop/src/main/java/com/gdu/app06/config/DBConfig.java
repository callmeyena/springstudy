...
@Configuration						// Spring Container에 Bean을 만들어 두는 곳
public class DBConfig {
	
	// DriverMangaerDataSource Bean
	@Bean
	...
	@Bean
	public TransactionInterceptor transactionInerceptor() {
	...
	/* 
	 	AOP 관련 핵심 용어 3가지
 	1. 조인포인트(JoinPoint)	: AOP를 적용시킬 수 있는 메소드 전체 		=> 전체집합(목록, 상세, 삽입, 수정, 삭제)
	 	2. 포인트컷(PointCut)	: 조인포인트 중에서 AOP를 동작시킬 메소드 	=> 부분집합(삽입, 수정, 삭제)
	...
	*/
	...
	public Advisor advisor() {
		...
		return new DefaultPointcutAdvisor(pointCut, transactionInerceptor());	// pointCut으로 등록된 메소드에 transactionInerceptor()를 동작시킨다.
	}
